...
public class PhotoController {
    ...
    @Operation(summary = "获取相册照片", description = "根据相册ID获取该相册下的所有照片")
    public Result<Page<Photo>> getPhotosByAlbum(
            @PathVariable Long albumId,
            @RequestParam(defaultValue = "1") Integer pageNum,
            @RequestParam(defaultValue = "10") Integer pageSize) {
        logger.info("【PhotoController】开始获取相册照片，相册ID: {}, 页码: {}, 每页数量: {}", albumId, pageNum, pageSize);
        try {
            Result<Page<Photo>> result = photoService.getPhotosByAlbum(albumId, pageNum, pageSize);
            logger.info("【PhotoController】相册照片获取成功，总数: {}", result.getData() != null ? result.getData().getTotal() : 0);
            return result;
        } catch (Exception e) {
            logger.error("【PhotoController】相册照片获取失败: {}", e.getMessage());
            throw e;
        }
    }
    ...
    @Operation(summary = "上传照片文件", description = "上传照片文件到服务器，返回URL")
    public Result<Map<String, String>> uploadPhoto(@RequestParam("file") MultipartFile file) {
        logger.info("【PhotoController】开始上传照片文件，文件名: {}", file.getOriginalFilename());
        try {
            String url = fileService.upload(file);
            logger.info("【PhotoController】照片文件上传成功，URL: {}", url);
            return Result.success(Map.of("url", url));
        } catch (Exception e) {
            logger.error("【PhotoController】照片文件上传失败: {}", e.getMessage());
            throw e;
        }
    }
    ...
    @PostMapping
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "创建照片", description = "创建照片记录，需要登录")
